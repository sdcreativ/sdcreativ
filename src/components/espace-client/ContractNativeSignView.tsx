...
export function ContractNativeSignView({ token }: { token: string }) {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch(`/api/espace-client/sign/contract/${encodeURIComponent(token)}`);
      const json = (await res.json()) as { contract?: ContractSummary; error?: string };
      if (!res.ok) throw new Error(json.error);
      setContract(json.contract ?? null);
      if (json.contract?.clientName) setSignerName(json.contract.clientName);
    } catch (err) {
  ...
  async function sendOtp() {
    ...
    try {
      const res = await fetch(
        `/api/espace-client/sign/contract/${encodeURIComponent(token)}/challenge`,
        { method: "POST" },
      );
  ...
  async function finalize() {
    ...
    try {
      const res = await fetch(
        `/api/espace-client/sign/contract/${encodeURIComponent(token)}/sign`,
        {
          method: "POST",
  ...
  return (
    <div className="mx-auto max-w-lg px-4 py-10">
      <div className="rounded-2xl border border-gray/40 bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 font-mono text-xs text-gray-text">{contract.reference}</p>
