...
import { sendInvoiceEmailApi } from "@/lib/invoices-api";
...
export function InvoiceEmailComposer({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      await sendInvoiceEmailApi(invoiceId, { subject: subject.trim(), body: body.trim() });
      onSent();
      onClose();
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="w-full max-w-lg rounded-2xl bg-white p-6 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-text">À {invoiceEmail}</p>
          </div>
        ...
        <div className="space-y-3">
          ...
          <p className="text-xs text-gray-text">
            Le récapitulatif de la facture (montant, lignes, reste dû), un bouton de paiement vers
            l&apos;espace client et le PDF seront inclus automatiquement dans l&apos;email.
          </p>
        </div>
