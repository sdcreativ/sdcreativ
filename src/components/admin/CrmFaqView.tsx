...
import {
  ...
  deleteFaqItemApi,
  fetchFaqItemsAdmin,
  importStaticFaqItemsApi,
  reorderFaqItemApi,
  updateFaqItemApi,
} from "@/lib/public-faq-api";
...
function recordToForm(r: PublicFaqItemRecord): Form {
  return {
    question: r.question,
    answer: r.answer,
...
export function CrmFaqView() {
  const { confirm, alert } = useDialog();
  const [items, setItems] = useState<PublicFaqItemRecord[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await fetchFaqItemsAdmin(localeFilter === "all" ? undefined : localeFilter);
      setItems(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Impossible de charger la FAQ.");
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  function openEdit(item: PublicFaqItemRecord) {
    setCreating(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (creating) {
        const item = await createFaqItemApi(form);
        setItems((prev) => [...prev, item].sort((a, b) => a.sortOrder - b.sortOrder));
        closeForm();
        setMessage("Question ajoutée.");
      } else if (editingId) {
        const item = await updateFaqItemApi(editingId, form);
  ...
  async function handleDelete(id: string, question: string) {
    const ok = await confirm({
      ...
      message: `« ${question.slice(0, 80)}… » sera retirée du site.`,
  ...
  async function handleToggleVisible(item: PublicFaqItemRecord) {
    setBusyId(item.id);
    try {
      const updated = await updateFaqItemApi(item.id, { isVisible: !item.isVisible });
  ...
  async function handleReorder(id: string, direction: "up" | "down") {
    setBusyId(id);
    try {
      await reorderFaqItemApi(id, direction);
      await load();
    } catch (err) {
      await alert({
        title: "Erreur",
  ...
  async function handleImportStatic() {
    const ok = await confirm({
      title: "Importer la FAQ statique ?",
      message: "Les 6 questions du code seront ajoutées en base (sans écraser les existantes).",
    ...
    setImporting(true);
    try {
      const result = await importStaticFaqItemsApi();
      await load();
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-text">
            Questions fréquentes sur l&apos;accueil et la page /faq (SEO JSON-LD inclus).
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-gray/60 bg-gray-light/30 px-6 py-12 text-center text-sm text-gray-text">
          Aucune question en base — le site affiche les données statiques du code.
        </div>
      ) : (
        <div className="space-y-3">
          {items.map((item, index) => (
            <article key={item.id} className={cn("rounded-2xl border bg-white p-4 shadow-sm", item.isVisible ? "border-gray/60" : "border-gray/40 opacity-70")}>
              ...
              <p className="mt-2 line-clamp-3 text-sm text-gray-text">{item.answer}</p>
              <div className="mt-3 flex flex-wrap gap-1.5">
                ...
                <button title="Haut" type="button" onClick={() => void handleReorder(item.id, "up")} disabled={busyId === item.id || index === 0} className="inline-flex items-center gap-1 rounded-lg border border-gray/60 px-2 py-1 text-xs font-medium hover:bg-gray-light disabled:opacity-40"><ArrowUp className="h-3 w-3" aria-hidden /></button>
                <button title="Bas" type="button" onClick={() => void handleReorder(item.id, "down")} disabled={busyId === item.id || index === items.length - 1} className="inline-flex items-center gap-1 rounded-lg border border-gray/60 px-2 py-1 text-xs font-medium hover:bg-gray-light disabled:opacity-40"><ArrowDown className="h-3 w-3" aria-hidden /></button>
                <button type="button" onClick={() => void handleDelete(item.id, item.question)} disabled={busyId === item.id} className="inline-flex items-center gap-1 rounded-lg border border-red-200 px-2 py-1 text-xs font-medium text-red-600 hover:bg-red-50 disabled:opacity-60"><Trash2 className="h-3 w-3" aria-hidden />Supprimer</button>
              </div>
            </article>
          ))}
        </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white p-6 shadow-xl" role="dialog" aria-modal="true">
            <h2 className="text-lg font-bold">{creating ? "Nouvelle question" : "Modifier la question"}</h2>
            <form onSubmit={(e) => void handleSubmit(e)} className="mt-4 space-y-4">
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-gray-text">Question</span>
                <input required value={form.question} onChange={(e) => setForm((p) => ({ ...p, question: e.target.value }))} className={fieldClass} />
              </label>
              <label className="block">
                <span className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-gray-text">Réponse</span>
                <textarea required rows={6} value={form.answer} onChange={(e) => setForm((p) => ({ ...p, answer: e.target.value }))} className={fieldClass} />
              </label>
              <label className="block">
