...
export async function sendContractForNativeSignature(input: {
  contractId: string;
  ...
  if (contract.signatureProvider === "yousign" && contract.esignExternalId) {
    throw new Error(
      "Ce contrat a déjà une demande Yousign active. Annulez-la ou attendez la finalisation.",
    );
  }
  ...
  const email =
    ...
    null;
  if (!email?.includes("@")) {
    throw new Error("Email signataire requis (client ou saisie manuelle).");
  }
  ...
  const signUrl = `${siteUrl}/espace-client/signer/contrat/${plainToken}`;
  ...
  const sent = await sendEmail({
    to: email,
    subject: `Signature — ${contract.reference}`,
    html: `
      <p>Bonjour${contract.clientName ? ` ${escapeHtml(contract.clientName)}` : ""},</p>
      <p>SD CREATIV vous invite à signer le contrat <strong>${escapeHtml(contract.reference)}</strong>
  ...
  if (!sent) {
    console.warn("[native-contract] invitation email non envoyé — lien généré quand même");
  }
...
export async function getContractByNativeSignToken(
  token: string,
  ...
  return withDb(async (query) => {
    ...
    return getContractById(rows[0].id);
  });
}

export async function signContractNative(input: {
  token: string;
  signerName: string;
  ...
  const contract = await getContractByNativeSignToken(input.token);
  ...
  const signatureHash = createHash("sha256")
    .update(
      `contract|${contract.id}|${signerName}|${signerEmail}|${signedAt.toISOString()}|${documentSha256}|${input.signatureData.slice(0, 1024)}`,
    )
    .digest("hex");

  const finalHtml = buildContractPdfHtml(contract, siteUrl, {
    signerName,
    signedAt: signedAt.toISOString(),
    signatureHash,
    signatureDataUrl: input.signatureData,
