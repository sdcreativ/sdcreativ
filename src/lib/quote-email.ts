...
export function buildQuoteEmailHtml(
  ...
  const lines = quote.lines.length
    ? quote.lines
        .map(
          (line) =>
            `<tr><td style="padding:6px 0;border-bottom:1px solid #e5e7eb">${escapeHtml(line.label)}</td><td style="padding:6px 0;border-bottom:1px solid #e5e7eb;text-align:right;font-weight:600">${formatQuoteAmount(line.amount)}</td></tr>`,
        )
        .join("")
    : "";
  ...
  return `<div style="font-family:system-ui,sans-serif;line-height:1.6;color:#111827;max-width:640px">
    ...
    <div style="margin:24px 0;padding:16px;background:#f8fafc;border-radius:12px;border:1px solid #e5e7eb">
      ...
      <p style="margin:0 0 12px"><strong>Total HT :</strong> ${formatQuoteAmount(quote.subtotal)}</p>
      ${lines ? `<table style="width:100%;font-size:14px">${lines}</table>` : ""}
      ${quote.estimateMin && quote.estimateMax ? `<p style="margin:12px 0 0;font-size:13px;color:#6b7280">Fourchette : ${formatQuoteAmount(quote.estimateMin)} – ${formatQuoteAmount(quote.estimateMax)}</p>` : ""}
    </div>
...
Nous revenons vers vous concernant notre devis ${quote.reference} (${formatQuoteAmount(quote.subtotal)}) envoyé il y a ${daysLabel} jour(s).
