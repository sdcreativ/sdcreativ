...
export function signFacebookOAuthState(userId: string): string {
...
export function verifyFacebookOAuthState(state: string): FacebookOAuthStatePayload | null {
  ...
  try {
    const payload = JSON.parse(base64UrlDecode(data)) as FacebookOAuthStatePayload;
    if (
      !payload.userId ||
      payload.purpose !== "facebook_page" ||
