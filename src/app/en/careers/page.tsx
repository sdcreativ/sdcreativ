...
import { careerFormCopy } from "@/i18n/form-copy";
import { getJobOffers, getJobSelectOptions } from "@/lib/public-careers-resolver";
import { createMetadata } from "@/lib/metadata";

export const metadata = createMetadata({
  title: "Careers",
  description:
    "Join SD CREATIV: field sales roles in Abidjan and Côte d'Ivoire. Apply online.",
  path: "/en/careers",
  locale: "en",
});

export default async function EnCareersPage() {
  const [jobOffers, jobSelectOptionsFr] = await Promise.all([
    getJobOffers(),
    getJobSelectOptions(),
  ]);

  const jobSelectOptions = jobSelectOptionsFr.map((option) => {
    if (option.value === "candidature-spontanee") {
      return { value: option.value, label: careerFormCopy.en.spontaneous };
    }
    const en = jobOffersEnById[option.value];
    return { value: option.value, label: en?.title ?? option.label };
  });

  const jobs = jobOffers.map((job) => {
    const en = jobOffersEnById[job.id];
    return en ? { ...job, ...en, id: job.id } : job;
  });
  ...
  return (
    <>
      <PageHero
        eyebrow="Careers"
        title={careersEn.title}
        highlight={careersEn.highlight}
      ...
      <section className="py-16 md:py-20">
        <div className="container mx-auto px-4 md:px-6 lg:px-8">
          ...
          <h2 className="mb-8 text-2xl font-bold text-foreground">{careersEn.openRolesTitle}</h2>
          {jobs.length === 0 && (
            <p className="mb-8 text-sm text-gray-text">
              No open roles right now — you can still send a spontaneous application below.
            </p>
          )}
            ...
            {jobs.map((job) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  ...
                  <Button href={`/en/careers?poste=${job.id}#candidature`} size="sm">
                    Apply
                  </Button>
                </div>
      ...
      >
        <div className="container mx-auto max-w-3xl px-4 md:px-6 lg:px-8">
          <Suspense fallback={<p className="text-center text-gray-text">Loading…</p>}>
            <CarriereForm locale="en" jobSelectOptions={jobSelectOptions} />
          </Suspense>
