...
/** Soft-delete sélectif de messages. Permission mail.write. */
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await softDeleteMailMessages(parsed.data.ids);

    await auditCrmAction({
      action: "delete",
      entityType: "mail_message",
      entityId: parsed.data.ids[0],
      summary: `${result.deleted} message(s) messagerie supprimé(s)`,
      metadata: {
        ids: parsed.data.ids,
    ...
    console.error("[api/admin/mail/messages/bulk-delete] POST", error);
