...
export async function PATCH(request: Request, { params }: Props) {
  ...
  try {
    ...
    const parsed = updatePublicTestimonialSchema.safeParse(body);
    ...
    const testimonial = await updatePublicTestimonial(id, parsed.data);
    if (!testimonial) {
    ...
    console.error("[api/admin/testimonials/id] PATCH", error);
    return NextResponse.json({ error: message }, { status: 400 });
  }
}
...
export async function DELETE(_request: Request, { params }: Props) {
  ...
  try {
    const { id } = await params;
    const deleted = await deletePublicTestimonial(id);
    if (!deleted) {
      return NextResponse.json({ error: "Témoignage introuvable." }, { status: 404 });
    }
    ...
    console.error("[api/admin/testimonials/id] DELETE", error);
    return NextResponse.json({ error: message }, { status: 400 });
  }
}
