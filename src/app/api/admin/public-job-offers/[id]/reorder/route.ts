...
import { reorderPublicJobOffer } from "@/lib/public-job-offers";
...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    ...
    const item = await reorderPublicJobOffer(id, parsed.data.direction);
    if (!item) return NextResponse.json({ error: "Offre introuvable." }, { status: 404 });

    revalidateCareersPages();
    return NextResponse.json({ item });
  } catch (error) {
    console.error("[api/admin/public-job-offers/[id]/reorder] POST", error);
